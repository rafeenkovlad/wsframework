...
class GetQueueStats extends MethodAbstract
{
    ...
    {
        return 'BrowserlessQueue.GetQueueStats';
    }
    ...
    protected static function process(int $workerId, int $connectionId, MethodDTO $methodDTO): array
    {
        ...
        $byStatus = [];
        foreach (BrowserlessJobStatus::cases() as $case) {
        ...
        $streamMap = [
            'browserlessJobs' => ['stream' => 'browserless_jobs'],
        ...
        try {
            $info = $natsChannel->getConsumerInfo(
                NatsStreamEnum::BROWSERLESS_JOB->getValue(),
                $natsChannel->getConsumerName(NatsSubjectEnum::BROWSERLESS_JOB->getValue())
            );
            $consumerInfo = [
        ...
        // Workers info
        $configured = (int)($_ENV['BROWSERLESS_QUEUE_COUNT_PROCESS'] ?? 1);
        $busy = $byStatus[BrowserlessJobStatus::BROWSERLESS_PROCESSING->value];
    ...
    {
        return 'Получить агрегированную статистику browserless-очереди: KV по статусам, JetStream streams, consumer и worker-процессы.';
    }
        ...
        return [
            ...
            'properties' => [
                ...
                'streams' => [
                    'type' => 'object',
                    'additionalProperties' => false,
                    'required' => ['browserlessJobs', 's3Upload'],
                    'description' => 'Состояние JetStream streams по стадиям.',
                    'properties' => [
                        'browserlessJobs' => $streamSchema,

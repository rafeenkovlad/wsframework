...
class PurgeAll extends MethodAbstract
{
    ...
    {
        return 'BrowserlessQueue.PurgeAll';
    }
    ...
    protected static function process(int $workerId, int $connectionId, MethodDTO $methodDTO): array
    {
        ...
        $kv = $bucketInterface->bucket('browserless_jobs_status');
        $entries = $kv->getAll();
        ...
        $bucketInterface->purgeStream(NatsSubjectEnum::BROWSERLESS_JOB->stream()->getValue());
    ...
    {
        return 'Удалить из KV-хранилища все browserless-задачи и очистить временные файлы.';
    }
